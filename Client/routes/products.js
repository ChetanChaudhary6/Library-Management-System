...
router.get('/cart', async (req, res) => {
  if (!req.user) {
    ...
    res.render('cart', { title: 'Book Store - Cart', items: data, subtotal: sub.toFixed(2), shipping: ship.toFixed(2), tax: (0.05 * sub).toFixed(2), totalcost: (1.05 * sub + ship).toFixed(2) });
  }
})
...
router.get('/checkout', async (req, res) => {
  if (!req.user) {
    ...
    res.render('checkout', { title: 'Book Store - Cart - Checkout', num: items.length, items: data, subtotal: sub.toFixed(2), shipping: ship.toFixed(2), tax: (0.05 * sub).toFixed(2), totalcost: (1.05 * sub + ship).toFixed(2) });
  }
});
...
router.post('/makepayment/:id', async (req, res) => {
  if (!req.user) {
    ...
    const order = await Order.findOne({ _id: mongoose.Types.ObjectId(req.params.id) });
    var instance = new Razorpay({ key_id: "rzp_test_yGtSO5e7p4JrnR", key_secret: "i6QCBno6F1R5R8uwrU4TyMUE" })
    var options = {
